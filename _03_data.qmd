...
p_pov <- ggplot(subset(baseline_long, variable=="Poverty Rate (%)"), aes(x=variable, y=value)) +
  geom_violin(fill="#ff7f00", color="#ff7f00", alpha=0.35, linewidth=0.6, trim=TRUE) +
...
p_unemp <- ggplot(subset(baseline_long, variable=="Unemployment Rate (%)"), aes(x=variable, y=value)) +
  geom_violin(fill="#fb9a99", color="#fb9a99", alpha=0.35, linewidth=0.6, trim=TRUE) +
  geom_boxplot(width=0.12, fill="white", color="grey30", alpha=0.7,
               outlier.size=0.8, outlier.alpha=0.5) +
  scale_y_continuous(labels=function(v) sprintf("%g%%", v)) +
...
p_pop <- ggplot(subset(baseline_long, variable=="Population"), aes(x=variable, y=value)) +
  geom_violin(fill="#8F8D87", color="#8F8D87", alpha=0.35, linewidth=0.6, trim=TRUE) +
  geom_boxplot(width=0.12, fill="white", color="grey30", alpha=0.7,
               outlier.size=0.8, outlier.alpha=0.5) +
...
p_inc <- ggplot(subset(baseline_long, variable=="Median Household Income ($)"), aes(x=variable, y=value)) +
  geom_violin(fill="#33a02c", color="#33a02c", alpha=0.35, linewidth=0.6, trim=TRUE) +
  geom_boxplot(width=0.12, fill="white", color="grey30", alpha=0.7,
               outlier.size=0.8, outlier.alpha=0.5) +
...
```{python}
#| label: fig-erd
#| fig-cap: "**Data Pipeline Architecture.** Traces the full pipeline from source ingestion through the normalized warehouse to analysis ready outputs."
#| echo: false
import graphviz
