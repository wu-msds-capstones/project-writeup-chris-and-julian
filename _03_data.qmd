...
The study period spans 2008 to 2023, excluding 2020. ACS suspended its 1 year estimates that year following COVID related disruptions to data collection. Because those estimates supply the occupational employment counts underlying the task group values, the task groups cannot be constructed for 2020. Across the fifteen retained years, SAIPE reports roughly 3,143 counties per year, giving a universe of 47,140 county year observations.[^counties]

[^counties]: County FIPS codes change over time as jurisdictions are redefined, so the count varies slightly by year. Connecticut replaced its eight counties with nine planning regions in 2022; the warehouse holds only the legacy counties, so Connecticut leaves the panel after 2021. The District of Columbia and Kalawao County, Hawaii, appear in SAIPE but not in the county reference file and are absent from the panel throughout.
...
The 47,140 county-year observations mentioned earlier represent the actual records that the data supports, not the ones that the models estimate. Two filters are applied to reduce the dataset.

Task group coverage serves as the primary constraint. The occupational employment counts that form the four task groups originate from ACS 1-year estimates, which the Census Bureau publishes only for areas with populations exceeding 65,000. Consequently, counties with populations below this threshold lack occupational estimates for any given year, rendering it impossible to construct task groups for them. Consequently, 12,087 county-year observations are retained across 848 counties, which constitutes approximately a quarter of the entire dataset.

Furthermore, the requirement for non-missing controls eliminates an additional 104 observations. Out of these 104, every single one is missing an unemployment rate, resulting in a coverage rate of 99.1 percent across the retained counties. Population, median household income, and poverty rate are all complete for the estimation sample. Consequently, the estimation sample comprises 11,983 county-year observations, and all the models reported in @sec-analysis and @sec-results are fitted on these same rows.

The 848 counties also serve as the unit of division for the machine learning models. Since a county appears in the panel up to fifteen times, random row splitting would inadvertently place the same county in both the training and test datasets. To address this issue, the split is performed by county, ensuring that every observation of a given county falls exclusively on one side. This approach results in 848, rather than 11,983, as the effective sample size for held-out evaluation. The rationale behind this grouped split is elaborated upon in @sec-analysis.

The population threshold significantly influences the scope of the results. Out of the 848 retained counties, approximately 84 percent of the United States population resides, yet they represent a minority of the total counties. These retained counties are characterized by higher population density, greater urbanization, and greater economic diversity compared to the counties that were excluded. Consequently, rural counties are underrepresented in the analysis. The restriction applies to both the regression coefficients and the machine learning results, resulting in partial dependence curves and importance rankings that describe the model’s behavior across counties with a population exceeding 65,000 rather than across all counties. Therefore, the findings should be interpreted as applicable to counties with a population exceeding 65,000 rather than to all counties. @sec-conclusions returns to this limitation and to one approach for addressing it in future work.
