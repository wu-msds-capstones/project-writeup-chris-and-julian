...
#| echo: false
#| warning: false
#| fig-cap: "**Distribution of Purchasing Power and Task Groups.** Across the 12,087 county year task group panel, before the 104 rows missing an unemployment rate are dropped, all shown as violin plots with an embedded box marking the median, interquartile range, and outliers. Purchasing power stands alone on the left, right skewed with a long upper tail and a heavy cluster of high end outliers. The four task groups sit together on the right on a shared percentage scale; non-routine cognitive holds the highest concentration of county employment."
%%R -i dists_df -w 10 -h 5.5 -u in -r 150
pp_vals <- subset(dists_df, variable=="Purchasing Power ($)")$value
pp_ylim <- c(0, quantile(pp_vals, 0.98, names=FALSE))
...
TASK_COLORS <- c("Routine Cognitive"=GREEN, "Routine Manual"=ORANGE,
                  "Non-Routine Cognitive"=BLUE, "Non-Routine Manual"=PINK)
...
#| echo: false
#| warning: false
#| fig-cap: "**Purchasing Power Before and After the Log Transform.** The level distribution is right skewed with a long upper tail; logging pulls the tail in and leaves a distribution close to symmetric."
...
ggplot(bw_df, aes(x=value, y=group, fill=group, alpha=component)) +
  ...
  labs(title="Between-County vs. Within-County Variation by Task Group",
       x="Share of Variance (Percent)", y=NULL) +
  theme(legend.position="bottom")
```
...
ggplot(state_df, aes(x=mean_pp, y=state_label)) +
  ...
  annotate("text", x=state_median, y=n_states + 0.9, label=median_label,
           size=3, color="grey30", hjust=0.5, vjust=0) +
...
#| echo: false
#| warning: false
#| fig-cap: "**Model Specification Comparison.** R squared climbs at each stage, but the raw target's failed diagnostics rule it out for inference; the log specification passes and carries the study's coefficient estimates forward."
...
ggplot(model_summary, aes(x=model, y=metric, fill=status)) +
  ...
  scale_fill_manual(values=c(neutral="#F1F1EE", fail=ORANGE, success=GREEN), guide="none") +
...
p2 <- ggplot(nn_reg_df, aes(x=epoch, y=loss, color=series)) +
  ...
  annotate("text", x=max(nn_reg_df$epoch)*0.5, y=ymax*0.5, label="Improved generalization",
           size=3.3, color=BLUE, fontface="italic") +
  scale_color_manual(values=c(Train=BLUE, Validation=ORANGE), guide="none") +
  scale_y_continuous(limits=c(0, ymax)) +
  scale_x_continuous(expand=expansion(mult=c(0.02, 0.28))) +
