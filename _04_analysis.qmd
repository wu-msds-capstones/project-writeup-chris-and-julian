...
vif_dot_df$spec <- factor(vif_dot_df$spec,
    ...
    labels=c("Group proportions, as estimated","Unnormalized employment totals"))

ggplot(vif_dot_df, aes(x=vif, y=group)) +
  # PINK for the thresholds: a different hue family from the green and orange
  # points, so the VIF = 10 line stays legible where it crosses the orange ones
  geom_vline(xintercept=5, linetype="dotted", linewidth=0.8, color=PINK) +
  geom_vline(xintercept=10, linetype="dashed", linewidth=0.9, color=PINK) +
  annotate("text", x=5, y=Inf, label="VIF = 5", hjust=-0.15, vjust=1.4, color=PINK, size=3.1) +
  annotate("text", x=10, y=Inf, label="VIF = 10", hjust=-0.15, vjust=1.4, color=PINK, size=3.3, fontface="bold") +
  ...
  scale_color_manual(values=c("Group proportions, as estimated"=GREEN,
                              "Unnormalized employment totals"=ORANGE)) +
...
#| echo: false
#| warning: false
#| fig-cap: "**Purchasing Power Before and After the Log Transform.** County year purchasing power across the analytical panel, in dollars at national average prices (A) and on the natural log scale (B). The black curve on each panel is a normal distribution fitted to that panel's mean and standard deviation, and the vertical lines mark the median (solid) and the mean (dashed). A) The level distribution is right skewed with a long upper tail, and its mean sits above its median. B) Logging pulls the tail in, the two lines converge, and the distribution tracks the normal curve closely."
...
# normal curve fitted to each distribution, plus median (solid) and mean (dashed);
# the gap between the two lines is the skew the panel titles report
lv <- subset(logt_df, dist==level_label)$value
lg <- subset(logt_df, dist==log_label)$value

p_level <- ggplot(data.frame(value=lv), aes(x=value)) +
  geom_histogram(aes(y=after_stat(density)), bins=50, fill=ORANGE) +
  stat_function(fun=dnorm, args=list(mean=mean(lv), sd=sd(lv)),
                color="#0B0B0B", linewidth=0.7) +
  geom_vline(xintercept=median(lv), color="#0B0B0B", linewidth=0.6) +
  geom_vline(xintercept=mean(lv), color="#0B0B0B", linewidth=0.6, linetype="dashed") +
  scale_x_continuous(labels=dollar_axis) +
  labs(title=level_label, x="Purchasing Power", y="Density") +
  theme(axis.text.y=element_blank(), axis.ticks.y=element_blank())

p_log <- ggplot(data.frame(value=lg), aes(x=value)) +
  geom_histogram(aes(y=after_stat(density)), bins=50, fill=GREEN) +
  stat_function(fun=dnorm, args=list(mean=mean(lg), sd=sd(lg)),
                color="#0B0B0B", linewidth=0.7) +
  geom_vline(xintercept=median(lg), color="#0B0B0B", linewidth=0.6) +
  geom_vline(xintercept=mean(lg), color="#0B0B0B", linewidth=0.6, linetype="dashed") +
  labs(title=log_label, x="Log Purchasing Power", y="Density") +
  theme(axis.text.y=element_blank(), axis.ticks.y=element_blank())
...
%%R -i influence_plot_df -i influence_range_df -w 9 -h 5.5 -u in -r 150
...
# Two solid fills rather than one color at two opacities, so each in-bar label
# clears the 4.5:1 contrast floor: green with ink 7.0, pink with white 5.7.
# BLUE is deliberately not used here; it fails at 4.5 against both white and ink.
...
We define each task group as its proportion of employment across the four categories, so the four values sum to one. As with the panel regression, that constraint makes `non_routine_cognitive_share` the omitted reference, so coefficients on the remaining task groups read as shifts toward each group and away from non-routine cognitive work.
...
ggplot(state_df, aes(x=mean_pp, y=state_label)) +
  ...
  annotate("text", x=low_mid, y=n_states / 2, label="Lower\nPurchasing\nPower",
           color=PP_BAD_RED, fontface="bold", size=5.5, hjust=0.5, vjust=0.5,
           alpha=0.6, lineheight=0.95) +
  annotate("text", x=high_mid, y=n_states / 2, label="Higher\nPurchasing\nPower",
           color=PP_GOOD_GREEN, fontface="bold", size=5.5, hjust=0.5, vjust=0.5,
           alpha=0.6, lineheight=0.95) +
...
The raw target residuals show the same systematic curvature already documented in @fig-diagnostics (Panels A and B), so the linear specification is poorly suited to the untransformed target here as well.
...
Logging the target resolves the pattern here the same way it did for the panel regression in @fig-diagnostics (Panels C and D): the residual spread evens out and the QQ plot tracks the diagonal more closely, so a linear model is appropriate on the logged target. That model reaches an in sample R² of 0.913 and a held out R², back transformed into dollars, of 0.896.
...
#| echo: false
#| tbl-cap: "**OLS Coefficients Across Model Specifications and Outcome Scales.** Coefficients from the baseline, full level, and full log Ordinary Least Squares fits."
...
style_table(GT(comparison_ols, rowname_col="Variable", groupname_col="Group")
  ...
  .tab_spanner(label="Log Purchasing Power", columns=["Coefficient","Percent per point"])
...
By comparison, a model with the task groups but without poverty and unemployment reaches an R² of 0.698. Task groups therefore carry substantial predictive information on their own, but the strongest performance comes from combining them with the economic controls. All three models use the same forest specification, logged outcome, and held out counties, so their performance is directly comparable.
