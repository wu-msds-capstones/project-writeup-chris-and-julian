...
A fourth diagnostic, Cook’s distance, evaluates whether any single county year disproportionately influences the level specification’s coefficients. None of the observations exceed the conventional threshold of 1.0, indicating that only one point warrants further investigation. The highest value recorded is 0.009. @fig-cooks-distance ranks the 10 counties with the largest Cook’s distance in any single year, considering each county’s maximum value across its years in the panel. Additionally, the figure plots each year’s value against that maximum. Counties achieve the ranking through two distinct mechanisms. Loudoun County, Virginia, the largest by far, achieves this through real year-to-year volatility. In contrast, several counties lower down, such as McKinley County, New Mexico, consistently maintain high values across nearly every year. Loudoun and Stafford County, Virginia, are both high purchasing power counties located in the Washington D.C. exurbs. Williamson County, Tennessee, is a high purchasing power Nashville exurb. Apache County, Arizona, is a low purchasing power county situated on the Navajo Nation. These counties are the same high-end outliers that @fig-dists already depicts in the purchasing power distribution.
...
No single point threatened the overall fit, so the more informative check was to refit the model after removing the most influential 1 percent of the panel, `{python} f"{n_excluded:,}"` county year observations. @tbl-influence-sensitivity reported the results. The task group coefficients changed by single digit to low double digit percentages, while the poverty coefficient remained relatively stable. This indicated that the main associations were not driven by a small number of extreme counties.
