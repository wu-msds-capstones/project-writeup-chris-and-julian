...
ggplot(bw_df, aes(x=value, y=group, fill=component)) +
  ...
  scale_fill_manual(values=c("Between Counties"="#2A78D6",
                             "Within Counties Over Time"="#EB6834"), name=NULL) +
...
year_panel <- function(d, ylab, lab_fn) {
  ...
  ggplot(d, aes(x=year, y=estimate)) +
    ...
    annotate("text", x=2020, y=covid_y, label="COVID-19\nno ACS data",
             ...
             fontface="bold") +
    geom_hline(yintercept=0, linetype="dashed", color="#8F8D87", linewidth=0.4) +
...
ggplot(vif_dot_df, aes(x=vif, y=group)) +
  ...
  scale_color_manual(values=c("Group proportions"="#1BAF7A",
                              "Employment counts"="#EB6834")) +
...
p_tasks <- ggplot(task_box_df, aes(x=variable, y=value, fill=variable, color=variable)) +
  ...
  scale_fill_manual(values=c("Purchasing Power"="#8659C4", "Routine Cognitive"="#51C3CC",
    "Routine Manual"="#006666", "Non-Routine Cognitive"="#FFAD65",
...
p_level <- dens_panel(lv, "Original Scale", level_skew, "Purchasing Power", TRUE, "#EB6834")
p_log   <- dens_panel(lg, "Log Scale", log_skew, "Log Purchasing Power", FALSE, "#1BAF7A")
...
p1 <- ggplot(resid_df, aes(x=fitted, y=resid)) +
  ...
  scale_fill_gradient(low="#E8EEF4", high="#2A78D6", guide="none") +
  geom_hline(yintercept=0, linewidth=0.4, linetype="dashed", color="grey40") +
  geom_smooth(method="loess", formula=y~x, se=FALSE, color="#F21A00", linewidth=0.9) +
...
p2 <- ggplot(resid_df, aes(sample=std_resid)) +
  stat_qq(color="#2A78D6", alpha=0.35, size=0.8) +
...
p3 <- ggplot(resid_log_df, aes(x=fitted_log, y=resid_log)) +
  geom_bin2d(bins=55, aes(fill=after_stat(count))) +
  scale_fill_gradient(low="#E8F4EE", high="#1BAF7A", guide="none") +
  geom_hline(yintercept=0, linewidth=0.4, linetype="dashed", color="grey40") +
  geom_smooth(method="loess", formula=y~x, se=FALSE, color="#F21A00", linewidth=0.9) +
  scale_y_continuous(breaks=scales::pretty_breaks(n=7)) +
...
p4 <- ggplot(resid_log_df, aes(sample=std_resid)) +
  stat_qq(color="#1BAF7A", alpha=0.35, size=0.8) +
  stat_qq_line(color="#0B0B0B", linewidth=0.8) +
  scale_y_continuous(breaks=scales::pretty_breaks(n=7)) +
...
p_level <- panel("Level Target", "#EB6834", "Fitted Purchasing Power",
                 "Actual Purchasing Power", "Level Target", TRUE)
p_log   <- panel("Log Target", "#1BAF7A", "Fitted Log Purchasing Power",
...
# the vectors follow the factor levels, which is the order the y axis draws them in
