...
#| echo: false
#| warning: false
#| fig-cap: "**Year Effects Relative to 2008.** Estimated year coefficients with 95 percent intervals, each the difference from the 2008 baseline shared by every county in that year. (A) The level specification, in dollars of purchasing power. (B) The log specification, as a percent difference. The series carries national price drift together with every other movement common to a year, which is why it cannot be read as a measure of inflation."
...
The unnormalized employment totals of @eq-totals cannot support a regression. A variance inflation factor measures how much a coefficient is destabilized by its correlation with the other inputs, and a value above 10 is the conventional threshold for concern. The four totals carry factors between 13 and 27, because all four are employment counts that scale with county size, and they move together and carry little independent information. The specification estimated fixes this twice over, as @fig-vif shows. Normalizing by the four group total, as @sec-data describes, removes the shared scale, and dropping non-routine cognitive as the reference group removes the constraint that the four proportions sum to one. Every factor on the estimated specification sits between 1.2 and 1.6.
...
ggplot(vif_dot_df, aes(x=vif, y=group)) +
  ...
  geom_vline(xintercept=5, linetype="dotted", linewidth=0.8, color=BROWN) +
  geom_vline(xintercept=10, linetype="dashed", linewidth=0.9, color=BROWN) +
  annotate("text", x=5, y=Inf, label="VIF = 5", hjust=-0.15, vjust=1.4, color=BROWN, size=3.1) +
  annotate("text", x=10, y=Inf, label="VIF = 10", hjust=-0.15, vjust=1.4, color=BROWN, size=3.3, fontface="bold") +
...
#| echo: false
#| warning: false
#| fig-cap: "**Regression Diagnostics, Raw vs. Log Specification.** Columns are the diagnostic, rows are the specification. Panels A and B show the raw specification, in dollars. Panels C and D show the log specification, in log dollars. Residuals are shown as binned density with a loess smoother, since individual points would overplot. The raw specification shows systematic curvature and a heavy QQ tail, while the log specification is substantially flatter and tracks the reference line more closely."
...
#| echo: false
#| warning: false
#| fig-cap: "**Binned Fit Accuracy, Level vs. Log Specification.** Fitted values are cut into twenty equal count bins, and each point compares a bin's mean prediction with its mean actual value; points on the line indicate no systematic bias in that range. On the level scale both tails sit above the line. On the log scale the points track it throughout."
...
Because none of these points individually threatens the fit, the more informative check is what happens to the coefficients when the most influential 1 percent of the panel, `{python} f"{n_excluded}"` county year observations, is removed and the model refit. @tbl-influence-sensitivity reports the result. The task group coefficients move by single digit to low double digit percentages, and the poverty coefficient barely moves at all. The associations this study leans on hardest are not artifacts of a handful of extreme counties. Unemployment rate is the exception: already the smallest and least precisely estimated of the controls, as @sec-results reports, its coefficient shifts by roughly a third. That sensitivity is consistent with unemployment rate carrying the weakest evidence of the three controls throughout this study, rather than with a problem specific to this check.
...
ggplot(bw_df, aes(x=value, y=group, fill=component)) +
  ...
  scale_fill_manual(values=c("Between Counties"=GREEN,
                             "Within Counties Over Time"=PINK), name=NULL) +
...
#| echo: false
#| warning: false
#| fig-cap: "**Mean Purchasing Power by State.** County year purchasing power, 2008 to 2023, ranked with the median of the state means marked. Texas sits at the median, supporting its use as the reference state. States with fewer than five counties in the panel are pooled and excluded from this ranking."
...
The raw target residuals show the same systematic curvature already documented in @fig-diagnostics (Panels A and B), and the linear specification is poorly suited to the untransformed target here as well.

#### Full specification on the logged outcome

The binned means in @fig-binned-scale showed an approximately exponential pattern, and we test whether logging the target makes a linear model appropriate here as well.
...
#| echo: false
#| warning: false
#| fig-cap: "**Random Forest Residuals vs. Predicted Values.** Residuals from the raw target forest on the held out test counties, both axes in dollars. No systematic curve or trend is visible across the range of predictions."
...
The gap narrows after regularization, but the network still does not beat the random forest or the log target linear model, both of which are far cheaper to fit and tune. This project is built to explain the relationship between task groups and purchasing power rather than to maximize predictive accuracy, and we stop tuning here and carry the linear and forest models forward as the primary results.
