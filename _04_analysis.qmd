...
p1 <- ggplot(resid_df, aes(x=fitted, y=resid)) +
  ...
  scale_fill_gradient(low="#E7F3F6", high=BLUE, guide="none") +
...
p3 <- ggplot(resid_log_df, aes(x=fitted_log, y=resid_log)) +
  geom_bin2d(bins=55, aes(fill=after_stat(count))) +
  scale_fill_gradient(low="#FDF9E5", high=GREEN, guide="none") +
  geom_hline(yintercept=0, linewidth=0.4, linetype="dashed", color="grey40") +
...
#| echo: false
#| tbl-cap: "Mean and standard deviation of held out R² across five fold grouped cross validation for Ordinary Least Squares and the random forest, both fit on the log target and scored on the dollar scale after back transformation."
