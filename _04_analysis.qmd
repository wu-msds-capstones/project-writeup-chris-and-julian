...
The analysis commenced by examining the unadjusted relationship between task groups and purchasing power. @fig-univariate plotted purchasing power against each of the four task groups across the panel. The two manual groups exhibited the most pronounced negative relationships, while non-routine cognitive tasks showed a positive correlation, and routine cognitive tasks remained relatively flat. None of these relationships were strong enough to stand alone, prompting the subsequent adjustment analysis. Interestingly, the same ordering persisted after the adjustment.
...
Year indicators captured the shared conditions across all counties within a specific year, encompassing events like the 2008 recession, its subsequent recovery, and the national price movements described in @sec-data. By including these indicators, we prevented attributing national changes to variations in county task groups. Since the year coefficients absorbed all common changes within a year, they were treated as nuisance parameters rather than evidence of increasing purchasing power. @fig-year-effects illustrated their pattern across the panel.
...
year_panel <- function(d, ylab, lab_fn) {
  ggplot(d, aes(x=year, y=estimate)) +
    ...
    # a tick on every year that carries a point, so the 2020 gap stays visible
...
A fourth diagnostic, Cook’s distance, evaluates whether any single county year disproportionately influences the level specification’s coefficients. None of the observations exceed the conventional threshold of 1.0, so no point warrants further investigation. The highest value recorded is 0.009. @fig-cooks-distance plots every county year by its leverage and standardized residual, with dashed curves tracing constant Cook’s distance. The conventional 0.5 and 1.0 contours fall well outside the plotted range, since reaching 1.0 at the highest observed leverage would require a standardized residual near `{python} f"{r_at_threshold:.0f}"`. The labeled points are the four counties with the largest values, each shown at its own most influential year. Loudoun and Stafford County, Virginia, are both high purchasing power counties located in the Washington D.C. exurbs. Williamson County, Tennessee, is a high purchasing power Nashville exurb. Apache County, Arizona, is a low purchasing power county situated on the Navajo Nation. These counties are the same high-end outliers that @fig-dists already depicts in the purchasing power distribution.

```{python}
#| label: fig-cooks-distance
#| echo: false
#| warning: false
#| fig-cap: "**Residuals Against Leverage With Cook's Distance Contours, Level Specification.** Every county year in the panel, plotted by leverage (its hat value, unitless) and standardized residual (in standard deviations). Dashed curves trace constant Cook's distance. Because the conventional 0.5 and 1.0 contours fall far outside the observed data, the two drawn levels are the 99th percentile cutoff used for the sensitivity refit and the observed maximum. The four most influential counties are labeled at their own most influential year."
...
ggplot() +
  ...
  # contour read as one. linetype carries the same split as color, so the two
  # levels stay separable under color vision deficiency.
  ...
  annotate("text", x=Inf, y=Inf, hjust=1.03, vjust=1.5, size=3.0,
           color="#52514E", lineheight=1.1,
           label=sprintf("n = 11,983    max Cook's D = %.4f\nReaching the conventional 1.0 threshold would\ntake a standardized residual near %.0f",
...
The level model indicated that a constant dollar association was insufficient to accurately describe the relationship. Logging purchasing power was employed to determine if the association was better represented proportionally, enabling a comparable shift in a task group to correspond to a larger dollar difference in counties with higher purchasing power. The @fig-log-transform visualization demonstrated how the transformation mitigated the right skew in purchasing power and laid the groundwork for reevaluating the residual pattern.
...
Refitting the same specification using log purchasing power resolved most of the problems identified in the level model. @fig-diagnostics showed less systematic curvature in the residuals, while the QQ points followed the reference line more closely after the transformation. Some tail deviation remained, but residual skew declined from 1.01 to 0.09. The log linear specification therefore provided the baseline for evaluating the more flexible models that followed.
...
# all gridlines off on every panel; the dashed zero line and the QQ reference
# line are the references that matter here
...
# $5,000 breaks from $30k to $80k. Offsets from the median were tried and
# reverted: anchoring every tick to the median produced labels like $4.47k and
# $9.47k that collided. The median keeps its own dotted line and its labeled
# annotation instead.
state_breaks <- seq(x_lo, x_hi, by=5000)
...
The reference group logic demonstrated in @fig-vif applies here as well. The additional concern was whether the year and state indicators introduced new collinearity after being added to the predictive specification. @tbl-vif-ml revealed that they did not, as all reported variance inflation factors remained below the conventional threshold of 5. This confirmed that the indicator blocks could be retained without materially destabilizing the estimated coefficients.
...
p1 <- ggplot(nn_init_df, aes(x=epoch, y=loss, color=series)) +
  ...
  # marker instead of running off the panel and into the y axis title. Raised to
  # the same offset panel B uses, so the two annotations sit level across the pair.
  annotate("label", x=best_init_epoch + 0.6, y=best_init_loss + ymax*0.45,
