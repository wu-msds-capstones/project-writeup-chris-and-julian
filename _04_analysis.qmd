...
#| echo: false
#| warning: false
#| fig-cap: "**Purchasing Power Against Each Task Group.** County year purchasing power, in dollars at national average prices, plotted against each task group's value as a percent of county employment, before any controls. Faint points are individual county years; solid points are means within twenty equal count bins. Each panel keeps its own x axis range, since the four groups span different shares of county employment. The manual groups decline most steeply and non-routine cognitive rises, an ordering the adjusted estimates preserve."
...
ggplot(state_df, aes(x=mean_pp, y=state_label)) +
  ...
  annotate("text", x=low_mid, y=n_states / 2, label="Lower\nPurchasing\nPower",
           color="#C0392B", fontface="bold", size=5, hjust=0.5, vjust=0.5,
           alpha=0.25, lineheight=0.95) +
  annotate("text", x=high_mid, y=n_states / 2, label="Higher\nPurchasing\nPower",
           color=GREEN, fontface="bold", size=5, hjust=0.5, vjust=0.5,
           alpha=0.25, lineheight=0.95) +
...
#| echo: false
#| warning: false
#| fig-cap: "**Highest Variance Inflation Factors, Year and State Indicators.** Ten highest values after removing the reference task group, with the conventional threshold of 5 marked. All values fall comfortably below it."
...
Removing the task groups reduces held out R² from 0.877 to `{python} f"{rf_no_exposure_r2:.3f}"`, a decline of 0.043.
...
By comparison, a model with the task groups but without poverty and unemployment reaches an R² of `{python} f"{rf_no_econ_r2:.3f}"`. Task groups therefore carry substantial predictive information on their own, but the strongest performance comes from combining them with the economic controls. All three models use the same forest specification, logged outcome, and held out counties, so their performance is directly comparable.
