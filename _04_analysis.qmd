...
ggplot(vif_dot_df, aes(x=vif, y=group)) +
  geom_vline(xintercept=5, linetype="dotted", linewidth=0.8, color="#0B0B0B") +
  geom_vline(xintercept=10, linetype="dashed", linewidth=0.9, color="#0B0B0B") +
  annotate("text", x=5, y=Inf, label="VIF = 5", hjust=-0.15, vjust=1.4, color="#0B0B0B", size=3.1) +
...
# One color for all three groups now; only alpha (between vs. within) varies,
# so text contrast only needs to track that, not which group the bar belongs to.
bw_text_df$text_color <- ifelse(bw_text_df$component=="Within Counties Over Time",
                                 "white", "#0B0B0B")

ggplot(bw_df, aes(x=value, y=group, alpha=component)) +
  ...
  scale_alpha_manual(values=c("Between Counties"=0.32,
                     ...
                     name=NULL, guide=guide_legend(override.aes=list(fill=BLUE))) +
...
# axis anchored at zero rather than centered on the median — a symmetric,
# median-centered range was tried and reverted because it pulled the left edge
# away from zero (to roughly state_median - half_range), a truncated axis that
# risks visually overstating differences between states (Tufte's Lie Factor).
# The median still gets its own tick, via state_breaks below, and its own
# labeled reference line, without needing the panel itself to look centered.
x_lo <- 0
x_hi <- pp_range[2] * 1.05

step <- pretty(c(0, x_hi - state_median), n=5)
step <- step[step > 0][1]
offsets_up <- seq(0, x_hi - state_median, by=step)
offsets_down <- seq(0, state_median - x_lo, by=step)
state_breaks <- sort(unique(c(state_median - offsets_down, state_median + offsets_up)))
...
#| echo: false
#| tbl-cap: "**Coefficients Across the Three OLS Specifications.** Baseline, full raw target, and full log target fits, with log target coefficients also converted to percent change per one percentage point shift. Task group coefficients are per unit of group proportion."
...
style_table(GT(comparison_ols)
  .fmt_currency(columns=["Baseline","With Controls"], decimals=0)
...
#| echo: false
#| tbl-cap: "**Model Specification Comparison.** Fit summary across the baseline, full raw target, and full log target Ordinary Least Squares specifications."
...
p1 <- ggplot(nn_init_df, aes(x=epoch, y=loss, color=series)) +
  ...
  annotate("label", x=best_init_epoch, y=best_init_loss + ymax*0.1,
           ...
           hjust=1, size=2.9, color="grey35", fill="#FCFCFB", linewidth=0,
