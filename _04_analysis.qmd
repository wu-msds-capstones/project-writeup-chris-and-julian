...
#| echo: false
#| warning: false
#| fig-cap: "**Variance Inflation Factors by Task Specification.** Shown on a linear scale, with thin and dashed reference lines marking the conventional thresholds of 5 and 10. Every unnormalized total sits well above the threshold of 10; every estimated factor sits near one. Non-routine cognitive has no estimated point because it serves as the reference group, dropped from the regression since the four proportions sum to one."
...
#| echo: false
#| warning: false
#| fig-cap: "**Purchasing Power Before and After the Log Transform.** (A) The level distribution is right skewed with a long upper tail. (B) Logging pulls the tail in and leaves a distribution close to symmetric."
...
A fourth diagnostic, Cook's distance, checks whether any single county year is disproportionately steering the level specification's coefficients. No observation crosses the conventional threshold of 1.0 for a point that should always be investigated; the largest value is 0.009. @fig-cooks-distance lists the ten highest, and the same few places recur across years: Loudoun County, Virginia and Stafford County, Virginia, both high purchasing power counties in the Washington D.C. exurbs; Williamson County, Tennessee, a high purchasing power Nashville exurb; and Apache County, Arizona, a low purchasing power county on the Navajo Nation. These are the same high end outliers @fig-dists already shows in the purchasing power distribution, rather than a new problem the earlier figure missed.
...
top_influential=(reg.nlargest(10, "cooks_d")
...
top_influential["label"]=(top_influential["county_name"]+", "+top_influential["state_name"]
                          +" ("+top_influential["year"].astype(str)+")")
influence_plot_df=top_influential[["label","cooks_d"]].sort_values("cooks_d")
...
#| echo: false
#| warning: false
#| fig-cap: "**Ten Highest Cook's Distance Values, Level Specification.** Every value falls far below the conventional threshold of 1.0 for a point requiring investigation; the same handful of high and low purchasing power counties recur across years."
%%R -i influence_plot_df -w 8.5 -h 4.2 -u in -r 150
influence_plot_df$label <- factor(influence_plot_df$label, levels=influence_plot_df$label)

ggplot(influence_plot_df, aes(x=cooks_d, y=label)) +
  geom_segment(aes(x=0, xend=cooks_d, yend=label), linewidth=1.1, color="#C8C8C5") +
  geom_point(size=4, color=BLUE) +
  geom_text(aes(label=sprintf("%.4f", cooks_d)), hjust=-0.3, size=3.4, color="#252525") +
  scale_x_continuous(limits=c(0, 0.012), expand=c(0,0)) +
  labs(title="Ten Highest Cook's Distance Values, Level Specification",
...
style_table(GT(sensitivity_df)
  .fmt_number(columns=["Full sample ($)","Excluding top 1% ($)"], decimals=0, use_seps=True)
