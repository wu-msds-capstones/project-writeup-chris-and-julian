...
#| echo: false
#| warning: false
#| fig-cap: "**Regression Diagnostics, Raw vs. Log Specification.** (A) Raw residuals versus fitted values and (B) the raw specification's QQ plot; (C) log residuals versus fitted values and (D) the log specification's QQ plot. Residuals are shown as binned density with a loess smoother, since individual points would overplot. The raw specification (A, B) shows systematic curvature and a heavy QQ tail, while the log specification (C, D) is substantially flatter and tracks the reference line more closely."
...
#| echo: false
#| tbl-cap: "Coefficients across the baseline, full raw target, and full log target OLS specifications, with log target coefficients also converted to percent change per one percentage point shift."
...
#| echo: false
#| tbl-cap: "Model specification comparison across the baseline, full raw target, and full log target Ordinary Least Squares fits."
...
#| echo: false
#| warning: false
#| fig-cap: "**Training and Validation Loss Across Model Specifications.** (A) In the initial network, validation loss reaches its minimum early, then rises while training loss keeps falling, the signature of overfitting. (B) After adding L2 regularization, heavier dropout, and a shorter early stopping patience, validation loss stays close to training loss throughout. Points mark the minimum validation loss in each panel; the same loss scale is used in both."
...
p1 <- ggplot(nn_init_df, aes(x=epoch, y=loss, color=series)) +
  ...
  geom_label(data=label_df1, aes(label=series, y=label_y), hjust=-0.1, size=3.4, fontface="bold",
             fill="#FCFCFB", label.size=0, label.padding=unit(0.08, "lines"), show.legend=FALSE) +
  annotate("label", x=best_init_epoch, y=best_init_loss + ymax*0.1,
           label=paste0("Minimum validation loss · epoch ", best_init_epoch),
           hjust=0, size=2.9, color="grey35", fill="#FCFCFB", label.size=0,
...
#| echo: false
#| tbl-cap: "Mean and standard deviation of held out R² across five fold grouped cross validation for Ordinary Least Squares and the random forest."
