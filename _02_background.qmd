...
#| echo: false
#| warning: false
#| fig-cap: "**Four Views of County Economic Conditions.** (A) Poverty rate and (B) unemployment rate describe economic distress. (C) Median household income describes nominal household resources. (D) Regional Price Parity shows how local prices vary relative to the national average, before the two are combined into purchasing power in later sections. All panels use the same year and geographic coverage. Continental United States, 2023. Each panel is clipped at the 2nd and 98th percentiles; counties without a value are shown in gray."
...
p_pov <- ggplot(context_sf) +
  ...
  scale_fill_gradient(low="#FCFCFB", high="#ff7f00", limits=c(pov_ctx_vmin, pov_ctx_vmax),
...
p_unemp <- ggplot(context_sf) +
  ...
  scale_fill_gradient(low="#FCFCFB", high="#fb9a99", limits=c(unemp_ctx_vmin, unemp_ctx_vmax),
...
p_inc <- ggplot(context_sf) +
  ...
  scale_fill_gradient(low="#FCFCFB", high="#33a02c", limits=c(inc_ctx_vmin, inc_ctx_vmax),
...
p_rpp <- ggplot(context_sf) +
  ...
  scale_fill_gradient(low="#FCFCFB", high="#1f78b4", limits=c(rpp_ctx_vmin, rpp_ctx_vmax),
                      oob=scales::squish, na.value="#EEEEEE",
