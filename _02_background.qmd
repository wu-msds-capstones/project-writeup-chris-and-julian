...
Autor developed a task-based approach to measure the variation in automation risk across different types of work. Initially, @Autor2003 distinguished between routine and non-routine tasks by demonstrating that technology impacts tasks differently based on their potential to be formalized into explicit procedures. @AcemogluAutor2011 further expanded his framework by treating tasks as production units and analyzing how workers with varying skills are distributed among them. This progression laid the conceptual foundation for the four task groups depicted in @fig-taskframework, linking technological advancements to shifts in the demand for different types of work.

Building upon this framework, @AutorDorn2013 applied the task-based approach to local labor markets. Their results showed that areas with higher initial concentrations of routine work experienced more pronounced employment polarization. Job growth occurred at both the upper and lower ends of the wage distribution while declining in the middle, suggesting that the local effects of technological change differed according to the task composition of each economy.
These findings established a basis for examining whether differences in county task composition were also associated with household purchasing power.

Classifying occupations by task content makes it possible to quantify differences in the composition of local employment. The occupational taxonomy developed by @AutorDorn2013 provided the basis for assigning employment to the four task groups used in this study. Their original measure scored occupations continuously using a routine task intensity index derived from ratings of occupational task content. We adapt this framework by grouping occupations according to their dominant task characteristics, as illustrated in @fig-taskframework. This classification allows differences in county task composition to be measured and compared across local economies.
...
# one label sits on a neighbor once repelled; nudge it down before repulsion runs
lab$nudge_y <- ifelse(lab$label == "Civil Engineers", -0.22, 0)
...
ggplot(occ_df, aes(x=x, y=y, color=group, shape=group)) +
  ...
  geom_point(data=lab, aes(x=x, y=y), inherit.aes=FALSE, shape=16, size=0.9,
             color="#0B0B0B") +
  # point.size matches the enlarged labeled marker so a label is pushed clear of its
  # own point rather than settling on top of it
  geom_text_repel(data=lab, aes(x=x, y=y, label=label), inherit.aes=FALSE,
                  size=2.3, fontface="bold", color="#0B0B0B", seed=510,
                  point.size=3, box.padding=0.45, point.padding=0.4,
                  nudge_y=lab$nudge_y,
                  min.segment.length=0.2, segment.color="#8F8D87",
...
Geographic scale presented an additional measurement challenge because broader units can obscure economic differences between nearby counties. Earlier studies often combined multiple counties into larger labor markets, potentially masking variation in local prices and household resources. @Curran2006 showed that accounting for geographic differences in the cost of living can alter comparisons of economic well being across places. We therefore measured purchasing power at the county level using household income adjusted for local price levels. We also tested whether its association with county task groups was better represented in dollar or proportional terms. This design extends prior research by examining task composition in relation to county level purchasing power rather than wages or employment alone.