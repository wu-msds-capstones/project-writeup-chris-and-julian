...
#| warning: false
#| label: fig-stability
#| fig-cap: "**Coefficient Stability Across Time Windows.** Log panel regression coefficients, as percent change in purchasing power per one percentage point shift out of non-routine cognitive work, refit on the pre pandemic and post pandemic windows alongside the full panel (diamond). Each window carries its own county year count in the legend, and the horizontal bar through each point is that window's 95% confidence interval. Only the full panel estimate is labeled with its value, at the left of each row. Standard errors clustered by county in every fit."
...
# green marks the preferred specification, the same role it carries elsewhere in the
# report; the full panel is the estimate the study quotes, the two windows flank it
