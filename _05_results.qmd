...
ggplot(compare_df, aes(x=est, y=group)) +
  ...
  geom_vline(xintercept=0, linetype="dashed", linewidth=0.4) +
  scale_color_manual(values=c("Task Groups and Year Only"=GREY, "With Controls"=BLUE)) +
...
style_table(GT(metrics_df)
  ...
  .tab_source_note("Neural network results are from this single split only; Table 4 reports cross validated results for the other two models."))
```
...
ggplot(area_df, aes(x=year, y=share, fill=group)) +
  ...
  annotate("text", x=2009.65, y=1.02, label="Census coding change", hjust=0, vjust=0, size=3.1, fontface="bold", color="#0B0B0B") +
  annotate("text", x=2020.15, y=1.02, label="No data", hjust=0, vjust=0, size=3.1, fontface="bold", color="#0B0B0B") +
...
@fig-taskchange-map shows this stability geographically, using the same two years as @tbl-rankcorr. Routine manual and non-routine manual, the two groups with almost no net movement in @fig-taskarea, show small changes that run in both directions across counties, consistent with variation that sits between counties rather than within them. Routine cognitive and non-routine cognitive, the pair whose national levels moved most in @fig-taskarea, show a change that is both larger and far more uniform in direction across the map, matching the within county movement identified in the variance decomposition of @sec-analysis.
...
#| echo: false
#| warning: false
#| fig-cap: "**County Task Group Change, 2010 to 2023.** Percentage point change in each task group's value, the same two years used in @tbl-rankcorr. Color scale shared across panels so magnitude is comparable across groups. Routine manual and non-routine manual show small, mixed change; routine cognitive and non-routine cognitive show larger, more uniformly signed change."
...
ggplot() +
  ...
  scale_color_manual(values=c("More At Risk"="#C0392B", "Less At Risk"=GREEN), name=NULL) +
  coord_sf(crs=st_crs(5070), datum=NA) +
  theme_void(base_size=11) +
  labs(title="Change in County Task Groups, 2010 to 2023") +
  theme(legend.position="bottom",
        plot.title=element_text(face="bold", size=13, hjust=0.5),
        plot.background=element_rect(fill="#FCFCFB", color=NA))
```
...
#| echo: false
#| warning: false
#| fig-cap: "**Task Group Coefficients Across Time Windows.** Log panel regression coefficients, as percent change in purchasing power per one percentage point shift out of non-routine cognitive work, refit on the pre pandemic and post pandemic windows alongside the full panel (diamond). Standard errors clustered by county in every fit."
