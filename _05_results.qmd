...
%%R -i compare_df -i compare_wide -w 9 -h 4.5 -u in -r 150 -b transparent
...
ggplot(compare_df, aes(x=est, y=group)) +
  ...
  geom_point(aes(color=spec, shape=spec), fill="white", size=3.2, stroke=1.0) +
  geom_text(aes(x=label_x, label=sprintf("%.1f%%", est), y=as.numeric(group)+label_nudge,
                color=spec),
            size=3.3, show.legend=FALSE, fontface="bold") +
  geom_vline(xintercept=0, linetype="dashed", linewidth=0.9, color="#0B0B0B") +
  scale_color_manual(values=c("Baseline"="#ff7f00", "With controls"="#33a02c")) +
...
Two measurements say how much the task groups contribute. The first removes them and refits: @fig-ablation shows what the random forest loses in held out R² when each block of features is taken away. Removing poverty and unemployment costs 0.178, more than four times the 0.042 lost by removing the task groups. The economic controls therefore carry more explanatory information, but the task groups still contribute beyond them, answering the question the objection above raised.
...
ggplot(ablation_df, aes(x=drop, y=model, fill=model)) +
  ...
  scale_fill_manual(values=c("Task Groups"="#33a02c", "Economic Controls"="#ff7f00"), guide="none") +
...
ggplot(importance_df, aes(x=drop, y=feature, fill=block)) +
  ...
  scale_fill_manual(values=c("Task Groups"="#33a02c", "Economic Controls"="#ff7f00",
                             "Structural Indicators"="#8F8D87")) +
...
%%R -i pdp_df -w 9 -h 3.5 -u in -r 150 -b transparent
pdp_levels <- c("Routine Cognitive","Routine Manual","Non-Routine Manual")
pdp_df$group <- factor(pdp_df$group, levels=pdp_levels)
PDP_COLORS <- c("Non-Routine Cognitive"="#fc8d62", "Non-Routine Manual"="#e78ac3", "Routine Cognitive"="#66c2a5", "Routine Manual"="#8da0cb")[pdp_levels]
...
ggplot(pdp_df, aes(x=x, y=y, color=group)) +
  ...
  # tight y padding so the curves fill the panel; the shared scale is kept so the
  # three declines stay comparable in magnitude
  scale_y_continuous(labels=function(v) sprintf("$%s", formatC(round(v), format="d", big.mark=",")),
                     expand=expansion(mult=c(0.02, 0.02))) +
  scale_x_continuous(labels=function(v) v * 100) +
...
ggplot(cv_folds, aes(x=x_spread, y=r2, color=model)) +
  ...
  scale_color_manual(values=c("Ordinary Least Squares"="#1f78b4",
                              "Random Forest"="#fb9a99")) +
...
#| echo: false
#| warning: false
#| fig-cap: "**Observed and Predicted Purchasing Power by Routine Cognitive Value.** Mean observed and predicted purchasing power across ten equal count bins of routine cognitive work, on the held out test counties, with predictions from the log target forest back transformed into dollars; each point marks a bin's mean routine cognitive value. The relationship rises then declines rather than moving monotonically; the forest reproduces this same shape rather than finding a different one."
...
ggplot(binned_long, aes(x=rc_pct, y=pp, color=series)) +
  ...
  scale_color_manual(values=c("Observed"=OBSERVED, "Predicted"="#1f78b4"), guide="none") +
...
TASKAREA_COLORS <- c("Non-Routine Cognitive"="#fc8d62", "Non-Routine Manual"="#e78ac3", "Routine Cognitive"="#66c2a5", "Routine Manual"="#8da0cb")
...
ggplot() +
  ...
  scale_color_manual(values=c("More At Risk"="#C0392B", "Less At Risk"="#1BAF7A"), name=NULL) +
  coord_sf(crs=st_crs(5070), datum=NA) +
...
window_colors <- c("2010 to 2019"="#1f78b4", "Full panel"=BLACK, "2021 to 2023"="#ff7f00")
