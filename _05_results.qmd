...
#| echo: false
#| warning: false
#| fig-cap: "**Where Purchasing Power Is Strained.** (A) Mean purchasing power by county, 2008 to 2023, in dollars at national average prices, clipped at the 2nd and 98th percentiles so extreme counties do not compress the scale. Counties without a purchasing power value are gray. (B) Density of mean purchasing power among counties grouped by their single largest task group. Continental United States only."
...
#| echo: false
#| warning: false
#| fig-cap: "**Partial Dependence on Each Task Group.** Random forest partial dependence of predicted purchasing power on each non-reference task group, computed on the held out test counties. Each curve moves one group across its observed range with all other features held at their values, so lines can span different segments of the x axis. The smooth, near monotonic declines match a proportional association."
%%R -i pdp_df -w 8 -h 4.5 -u in -r 150
...
ggplot(pdp_df, aes(x=x, y=y, color=group)) +
  ...
  scale_color_manual(values=PDP_COLORS, name=NULL) +
  scale_y_continuous(labels=function(v) sprintf("$%s", formatC(round(v), format="d", big.mark=","))) +
...
ggplot(area_df, aes(x=year, y=share, fill=group)) +
  ...
  annotate("text", x=2009.65, y=0.97, label="Census coding change", hjust=0, vjust=0.5, size=3.1, fontface="bold", color="#0B0B0B") +
  annotate("text", x=2020.15, y=0.97, label="No data", hjust=0, vjust=0.5, size=3.1, fontface="bold", color="grey35") +
  ...
  scale_y_continuous(labels=function(v) sprintf("%.0f%%", v*100), breaks=seq(0, 1, 0.25),
                      limits=c(0, 1), expand=expansion(mult=c(0.02, 0))) +
  scale_x_continuous(breaks=2008:2023, limits=c(2008, 2023),
...
style_table(GT(rankcorr_df)
  ...
  .cols_width(cases={"Task group": "140px", "Rank correlation, 2010 to 2023": "120px"})
...
@fig-taskchange-arrows collapses task group change from 2008 to 2023 into a single map by using the two axes that already structure the four task groups in @fig-taskframework: horizontal position shows movement between manual and cognitive work, vertical position shows movement between routine and non-routine work. This figure alone starts at 2008 rather than 2010; every other change measurement in this study starts at 2010 specifically to avoid the Census occupation coding change described in @sec-data, which can inflate apparent movement for routine cognitive and non-routine cognitive. Reading this map, keep in mind that some of the movement shown for those two groups reflects that coding change rather than a real shift in county employment. Most counties point up and to the left, toward non-routine cognitive work, consistent with routine cognitive and non-routine cognitive being the pair that moved most in @fig-taskarea. A county's color marks whether it moved toward or away from non-routine cognitive work, the reference group and the only one of the four positively associated with purchasing power, so counties colored red moved toward the three groups this study associates with lower purchasing power.

```{python}
#| label: taskchange-arrows-data
#| include: false
# this figure alone runs 2008 to 2023 rather than 2010 to 2023; a separate wide/common
# pair keeps this from disturbing tbl-rankcorr and fig-taskchange-map, which deliberately
# start at 2010 to avoid the Census occupation coding change
wide_2008=df[df["year"]==2008].set_index("county_fips")
common_08=wide_2008.index.intersection(wide_2023.index)
change_wide_08=(wide_2023.loc[common_08, list(area_names.keys())]
                -wide_2008.loc[common_08, list(area_names.keys())])*100
change_wide_08.columns=[area_names[c] for c in change_wide_08.columns]

manual_change_arrows=change_wide_08["Routine Manual"]+change_wide_08["Non-Routine Manual"]
nonroutine_change_arrows=change_wide_08["Non-Routine Cognitive"]+change_wide_08["Non-Routine Manual"]

arrow_shift_df=pd.DataFrame({
    "fips": change_wide_08.index.astype(str).str.zfill(5),
...
arrow_shift_df["risk_direction"]=np.where(
    change_wide_08["Non-Routine Cognitive"].values<0, "More At Risk", "Less At Risk")
...
#| echo: false
#| warning: false
#| fig-cap: "**Change in County Task Composition, 2008 to 2023.** Each arrow starts at a county's centroid; direction combines movement on the manual-cognitive axis (horizontal) and the routine-non-routine axis (vertical) from @fig-taskframework. Color marks whether a county moved toward or away from non-routine cognitive work, the regression's reference group and the only one of the four task groups positively associated with purchasing power. Arrow length is capped at the 95th percentile of shift magnitude so a handful of outlier counties do not dominate the figure. Unlike other change measurements in this study, this window starts at 2008 rather than 2010, so some routine cognitive and non-routine cognitive movement may reflect the Census occupation coding change rather than a real shift."
...
ggplot() +
  ...
  labs(title="Change in County Task Composition, 2008 to 2023") +
  theme(legend.position="bottom",
        plot.title=element_text(face="bold", size=13, hjust=0.5),
        plot.background=element_rect(fill="#FCFCFB", color=NA))
```
