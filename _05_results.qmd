...
#| echo: false
#| warning: false
#| fig-cap: "**Two Conventional Distress Measures, by County.** Mean poverty rate and mean unemployment rate, 2008 to 2023, in percent. Each panel uses its own color scale, clipped at the 2nd and 98th percentiles so extreme counties do not compress it. Counties without a value are shown in gray. Continental United States only."
...
The association also has a clear geographic pattern. @fig-afford-map maps mean purchasing power by county across the study period, using all counties with a purchasing power value rather than the analytical panel alone, since the outcome requires only income and a price parity. Purchasing power runs highest along the metropolitan Northeast corridor, across parts of the upper Midwest, and in pockets of the mountain West, while the lowest values concentrate across the rural South and the southern border region. The bottom panel relates this geography to local task groups by grouping counties according to whichever task group holds the largest share of local employment. The 664 counties where non-routine cognitive work dominates reach both higher and more varied purchasing power than the 97 dominated by non-routine manual work or the 87 dominated by routine manual work, which both cluster tightly at the low end. No county in the panel has routine cognitive work as its largest group, so that group does not appear. The map shows the outcome side of the association the coefficients estimate, and the density panel shows the same association holding at the level of a county's single largest task group.
...
p1 <- ggplot(afford_sf) +
  ...
  theme_void() +
  labs(title="Mean Purchasing Power by County") +
  theme(plot.title=element_text(face="bold", size=12, color="#0B0B0B", hjust=0.5),
...
#| echo: false
#| warning: false
#| fig-cap: "**Held Out R Squared Across Cross Validation Folds.** For the log target linear model and the log target random forest, with the mean marked. Both are scored on the dollar scale after back transformation. The gap between the model means is small relative to the spread across folds."
%%R -i cv_folds -i cv_means -w 8 -h 4 -u in -r 150
cv_means$mean_label <- sprintf("%.3f", cv_means$mean_r2)
cv_means$label_x <- ifelse(cv_means$model=="OLS (log target)", 0.62, 2.38)
...
ggplot(cv_folds, aes(x=model, y=r2, color=model)) +
  ...
  scale_color_manual(values=c("OLS (log target)"=GREEN,
                              "Random forest (log target)"=PINK)) +
  scale_x_discrete(expand=expansion(add=0.6)) +
  labs(title="Held Out R Squared Across Cross Validation Folds",
       x=NULL, y="Held Out R Squared ($ Scale)") +
  guides(color="none")
```
...
style_table(GT(metrics_df)
  .tab_header(title="Held Out Performance, Single Grouped Split")
...
#| echo: false
#| warning: false
#| fig-cap: "**Actual vs. Predicted Purchasing Power by Routine Cognitive Share.** Mean actual and predicted purchasing power across ten equal count bins of routine cognitive work, on the held out test counties, with predictions from the log target forest back transformed into dollars. The relationship rises then declines rather than moving monotonically; the forest reproduces this same shape rather than finding a different one."
...
ggplot(binned_long, aes(x=rc_pct, y=pp, color=series)) +
  annotate("text", x=min(binned_long$rc_pct), y=max(binned_long$pp)*0.975,
           label=rf_stat_label, hjust=0, size=3.4, color=PINK, fontface="bold") +
  ...
  labs(title="Actual vs. Predicted Purchasing Power by Routine Cognitive Share",
       x="Routine Cognitive Group Value (Bin Mean)", y="Mean Purchasing Power")
```
...
#| echo: false
#| warning: false
#| fig-cap: "**Loss in Held Out R Squared When Each Feature Block Is Removed.** From the log target random forest, relative to the full model's 0.876. Removing poverty and unemployment costs more than four times as much as removing the task groups."
...
ggplot(ablation_df, aes(x=drop, y=model, fill=model)) +
  ...
  labs(title="Loss in Held Out R Squared When Each Feature Block Is Removed",
       x="Loss in Held Out R Squared", y=NULL)
```
...
#| echo: false
#| warning: false
#| fig-cap: "**Feature Importance by Grouped Permutation.** On the held out test counties, measured as the drop in R-squared when each feature or block is shuffled. The task groups are permuted jointly, as are the year and state indicator blocks. Larger drops indicate features the model relies on more."
...
ggplot(importance_df, aes(x=drop, y=feature, fill=block)) +
  ...
  labs(title="Feature Importance by Grouped Permutation",
       x="Drop in Held Out R Squared When Permuted", y=NULL, fill=NULL) +
  theme(legend.position="bottom")
```
...
PDP_COLORS <- c("Routine Cognitive"=GREEN, "Routine Manual"=ORANGE, "Non-Routine Manual"=PINK)
...
#| echo: false
#| warning: false
#| fig-cap: "**Average County Balance of Work by Year.** Taken as an unweighted mean across panel counties. The balance across the four groups is nearly constant over the study period. The bold dashed line marks the Census occupation coding change between 2009 and 2010. The lighter dashed line at 2020 marks the year ACS 1 year estimates were not published; the narrow break in the bands at that point reflects the missing year rather than a data error."
...
# manual black halo behind the white overlaid labels for contrast against every band color
halo <- expand.grid(dx=c(-0.09,0,0.09), dy=c(-0.009,0,0.009))
halo <- halo[!(halo$dx==0 & halo$dy==0),]

p <- ggplot(area_df, aes(x=year, y=share, fill=group)) +
  geom_area(data=pre_df, stat="identity", position="stack", linewidth=0) +
  geom_area(data=post_df, stat="identity", position="stack", linewidth=0) +
  ...
  annotate("text", x=2009.5, y=1.16, label="Census coding change", hjust=0.5, vjust=0, size=3.1, fontface="bold", color="#0B0B0B") +
  annotate("text", x=2020, y=0.5, label="No ACS 1-year\nestimates", size=3, color="grey45", lineheight=0.95)

for (i in seq_len(nrow(halo))) {
  halo_df <- label_df
  halo_df$x <- label_year + halo$dx[i]
  halo_df$y <- halo_df$mid + halo$dy[i]
  p <- p + geom_text(data=halo_df, aes(x=x, y=y, label=group),
                      hjust=0.5, size=3.3, fontface="bold", color="black")
}

p +
  geom_text(data=label_df, aes(x=label_year, y=mid, label=group),
            hjust=0.5, size=3.3, fontface="bold", color="white") +
  scale_fill_manual(values=TASKAREA_COLORS, guide="none") +
  scale_y_continuous(labels=function(v) sprintf("%.0f%%", v*100), limits=c(0, 1.002), expand=c(0,0)) +
  scale_x_continuous(breaks=2008:2023, limits=c(2008, 2023.3), expand=c(0,0)) +
  coord_cartesian(clip="off") +
  labs(title="Average County Balance of Work by Year",
       x="Year", y="Mean Group Value Across Counties") +
  theme(axis.text.x=element_text(angle=0, hjust=0.5),
        plot.margin=margin(34, 5.5, 5.5, 5.5))
```
