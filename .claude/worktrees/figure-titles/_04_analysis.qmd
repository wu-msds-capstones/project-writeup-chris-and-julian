...
GROUP_COLORS <- c("Routine Cognitive"=GREEN, "Routine Manual"=ORANGE,
                   "Non-Routine Cognitive"=BLUE, "Non-Routine Manual"=PINK)
...
#| echo: false
#| warning: false
#| fig-cap: "Variance inflation factors for the two specifications, on a linear scale. Thin and dashed reference lines mark the conventional thresholds of 5 and 10. Every unnormalized total sits well above the threshold of 10; every estimated factor sits near one. Non-routine cognitive has no estimated point because it serves as the reference group, dropped from the regression since the four proportions sum to one."
...
ggplot(vif_dot_df, aes(x=vif, y=group)) +
  geom_vline(xintercept=5, linetype="dotted", linewidth=0.35, color="grey65") +
  geom_vline(xintercept=10, linetype="dashed", linewidth=0.7, color="grey35") +
  ...
  scale_x_continuous(limits=c(0, 30), breaks=c(1, 5, 10, 20, 30)) +
  ...
  labs(title="Variance Inflation Factors by Task Specification",
       subtitle="Raw employment totals produce substantially higher VIFs",
...
#| echo: false
#| warning: false
#| fig-cap: "Distributions across the 12,087 county year task group panel, before the 104 rows missing an unemployment rate are dropped, all shown as density distributions with a dashed line marking each median. Purchasing power stands alone on the left, right skewed with a long upper tail. The four task groups sit together on the right on a shared percentage scale; non-routine cognitive holds the highest concentration of county employment."
%%R -i dists_df -w 10 -h 5.5 -u in -r 150
make_dens <- function(data, var_name, fill_color, show_y=FALSE, label_fn=percent,
                       xlim=c(0.05, 0.65), median_label_fn=function(m) sprintf("Median: %.0f%%", m*100)) {
  d <- subset(data, variable==var_name)
  med <- median(d$value)
  ggplot(d, aes(x=value)) +
    geom_density(fill=fill_color, color=fill_color, alpha=0.35, linewidth=0.6) +
    geom_vline(xintercept=med, linetype="dashed", linewidth=0.5, color="grey30") +
    annotate("text", x=med, y=Inf, label=median_label_fn(med),
             hjust=-0.1, vjust=1.5, size=2.9, color="grey30") +
    scale_x_continuous(labels=label_fn) +
    coord_cartesian(xlim=xlim) +
    labs(title=var_name, x=NULL, y=if (show_y) "Density" else NULL) +
    theme(plot.margin=margin(3, 6, 3, 6))
}

pp_vals <- subset(dists_df, variable=="Purchasing Power ($)")$value
pp_xlim <- c(0, quantile(pp_vals, 0.98, names=FALSE))
pp_median_label <- function(m) sprintf("Median: $%s", formatC(round(m), format="d", big.mark=","))

p_pp  <- make_dens(dists_df, "Purchasing Power ($)", GREY, show_y=TRUE,
                    label_fn=dollar_axis, xlim=pp_xlim, median_label_fn=pp_median_label)
p_rc  <- make_dens(dists_df, "Routine Cognitive", GREEN, show_y=TRUE)
p_rm  <- make_dens(dists_df, "Routine Manual", ORANGE)
p_nrc <- make_dens(dists_df, "Non-Routine Cognitive", BLUE)
p_nrm <- make_dens(dists_df, "Non-Routine Manual", PINK)
...
ggplot(bw_df, aes(x=value, y=group, fill=component)) +
  ...
  labs(title="Between-County vs. Within-County Variation by Task Group",
       x="Share of Variance (Percent)", y=NULL, fill=NULL) +
  theme(legend.position="bottom")
```
...
(GT(comparison_ols)
  ...
  .tab_source_note("Task group coefficients are per unit of group proportion; percent change is per one percentage point."))
```
...
ggplot(model_summary, aes(x=model, y=metric, fill=status)) +
  ...
  labs(title="Model Specification Comparison",
       x=NULL, y=NULL) +
...
p1 + p2 +
  plot_annotation(title="Training and Validation Loss, Before and After Regularization",
                  theme=theme(plot.title=element_text(face="bold", size=13, hjust=0.5)))
```
...
(GT(cv_summary)
  .tab_header(title="Five Fold Grouped Cross Validation, Held Out R Squared")
  .fmt_number(columns=["Mean R squared","SD"], decimals=4)
  .cols_align(align="right", columns=["Mean R squared","SD"])
  .tab_source_note("Both models scored on the dollar scale after back transformation from the log target."))
```
