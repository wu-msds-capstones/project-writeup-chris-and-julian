...
ggplot(importance_df, aes(x=drop, y=feature)) +
  ...
  labs(title="Feature Importance by Grouped Permutation",
       x="Drop in Held Out R Squared When Permuted", y=NULL)
```
...
full_grid=pd.DataFrame({"year": range(2008, 2024)}).merge(
    pd.DataFrame({"group": list(area_names.values())}), how="cross")
area_df=full_grid.merge(area_df, on=["year","group"], how="left")
```
...
label_df <- area_df[area_df$year==2023,]
...
ggplot(area_df, aes(x=year, y=share, fill=group)) +
  annotate("rect", xmin=2019.5, xmax=2020.5, ymin=-Inf, ymax=Inf, fill="#E8E8E5", alpha=0.65) +
  geom_area(position="stack", linewidth=0) +
  geom_vline(xintercept=2009.5, linetype="dashed", linewidth=0.6, color="grey45") +
  annotate("text", x=2009.65, y=0.96, label="Census coding change", hjust=0, vjust=1, size=3.1, color="grey35") +
  annotate("text", x=2020, y=0.5, label="No ACS 1-year\nestimates", size=3, color="grey45", lineheight=0.95) +
  geom_text(data=label_df, aes(x=2023.4, y=mid, label=group, color=group),
            hjust=0, size=3.15, fontface="bold", show.legend=FALSE) +
  scale_fill_manual(values=TASKAREA_COLORS, guide="none", na.value=NA) +
  scale_color_manual(values=TASKAREA_COLORS, guide="none") +
  scale_y_continuous(labels=function(v) sprintf("%.0f%%", v*100), limits=c(0, 1), expand=c(0,0)) +
  scale_x_continuous(breaks=2008:2023, limits=c(2008, 2023), expand=expansion(mult=c(0.01, 0.16))) +
  labs(title="Average County Task Composition by Year",
